...
componentDidMount() {
        fetch('http://101.200.51.53:8080/SSLearningTeam/course/user/getAllQuestion?courseId=1&courseType=listeningcourse')
            ...
            .then((responseJson) => {
                this.setState({
                    loaded: true,
                    title : responseJson.info[8].questionContent,
                    A : responseJson.info[8].answerA,
                    B : responseJson.info[8].answerB,
                    C : responseJson.info[8].answerC,
                    D : responseJson.info[8].answerD,
                    reason : responseJson.info[8].answerReason,
                    answerReal : responseJson.info[8].answerReal,
                });
  ...
  render() {
    ...
    <Image style={styles.headImg} source={require('../imgs/xiazai.png')}/>
     ...
     <Text style={{fontSize:this.state.text_value,color:'black',marginLeft:20,marginBottom:10,fontWeight:'bold',}}>1.{this.state.title}</Text>
      ...
      <TouchableOpacity onPress={this.TiJiao.bind(this)}>
      <View style={{width:80,height:30,backgroundColor:'green',marginLeft:300}}>
      <Text style={{fontSize:20,fontWeight:'bold',textAlign:'center',color:'white'}}>提交</Text>
      </View>
       </TouchableOpacity>
